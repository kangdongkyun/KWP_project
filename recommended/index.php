        ...
        <div class="section">
            <div class="base">
                <div class="selectbar">
                    <h2>추천도서</h2>
                    <ul>
                        <li><a href="./index.php?mode=<?php echo "문학상수상도서"; ?>">문학상 수상도서</a></li>
                        <li><a href="./index.php?mode=<?php echo "이용자추천도서"; ?>">이용자 추천도서</a></li>
                    </ul>

                </div>
                <div class="free">
                    <?php if(!isset($_GET['mode']))$_GET['mode']="문학상수상도서" ?>
                    <h1><?php echo "$_GET[mode]"; ?></h1>
...
<!-- class = 'table table-hover' -->
                            <tbody>
                                ...
                                $num_per_page = 5;
                                if($_GET['mode']==="문학상수상도서") {
                                    $row = mysql_fetch_row(mysql_query("select count(*) from book  where prize = 1  order by recommended DESC;"));
                                }
                                else if ($_GET['mode']==="이용자추천도서"){
                                    $row = mysql_fetch_row(mysql_query("select count(*) from book order by recommended DESC;"));
                                }
                                ...
                                if($_GET['mode']==="문학상수상도서") {
                                    $sql= "select * from book where prize = 1 limit $offset,$num_per_page;";
                                }
                                else if ($_GET['mode']==="이용자추천도서"){
                                    $sql= "select * from book  order by recommended DESC limit $offset,$num_per_page;";
                                }
                        ...
                        <nav aria-label="Page navigation example">
                          <ul class="pagination justify-content-center">
                            ...
                            <?php
                                ...
                                while($i<$total_page){
                                    $i++;
                                    echo "<li class=\"page-item\"><a class=\"page-link\" href=\"./index.php?page=".$i."&mode=".$_GET['mode']."\">$i</a></li>";
